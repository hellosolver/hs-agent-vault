...
8. **Git Gate Approval**: Inspect git status, changed files, staged files, branch name, and commit readiness. Only Developer/DevOps may push to `develop`; all other agents are forbidden from git push. Staging/main require explicit human manual approval.
...
12. **Manual Approval Action Lock**: Do not require manual approval for `develop` push after local gates. Record explicit human manual approval before `staging` or `main` promotion.
